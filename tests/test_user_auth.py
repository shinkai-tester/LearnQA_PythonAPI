...
class TestUserAuth:
    ...
    def setup(self):
        ...
        self.user_id_auth_method = response1.json()["user_id"]
    ...
    def test_auth_user(self):
        ...
        assert "user_id" in response2.json(), "There is no user id in the 2nd response"
        user_id_check_method = response2.json()["user_id"]

        assert self.user_id_auth_method == user_id_check_method, "User id from auth method is not equal to user id from check method"
